...
# writer = tf.summary.FileWriter("./log/or_logs", sess.graph_def)
...
while score > MAX_ERROR:
    ...
    if i % 10000 == 0:
        sess.run(trainStep, feed_dict={x: OR_X, y: OR_Y})
        print('iteration: ', i)
        print('Output: ')
        print(sess.run(output, feed_dict={x: OR_X, y: OR_Y}))
        score = sess.run(cost, feed_dict={x: OR_X, y: OR_Y})
        print('cost: ', score)
